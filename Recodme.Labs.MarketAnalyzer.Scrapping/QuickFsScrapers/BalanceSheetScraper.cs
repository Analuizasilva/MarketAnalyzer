        ...
        public async Task<List<ExtractedValues>> ScrapeBalanceSheet(string ticker)
{
    ...
    for (var i = 1; i < numberOfColumns; i++)
    {
        ...
        for (int x = 2; x < numberOfRows; x++)
        {
            ...
            {
                var index = (htmlNodes[a * numberOfColumns].InnerLength);
                balanceSheet.Year = yearNumber;
                balanceSheet.CashEquivalents = valuesFinalList[index];
                balanceSheet.ShortTermDebt = valuesFinalList[index];
                balanceSheet.AccountsReceivable = valuesFinalList[index];
                balanceSheet.Inventories = valuesFinalList[index];
                balanceSheet.OtherCurrentAssets = valuesFinalList[index];
                balanceSheet.TotalCurrentAssets = valuesFinalList[index];

                balanceSheet.Investiments = valuesFinalList[index];
                balanceSheet.PropertyPlantAndEquipment = valuesFinalList[index];
                balanceSheet.Goodwill = valuesFinalList[index];
                balanceSheet.OtherIntangibleAssets = valuesFinalList[index];
                balanceSheet.OtherAssets = valuesFinalList[index];
                balanceSheet.TotalAssets = valuesFinalList[index];

                balanceSheet.AccountsPayable = valuesFinalList[index];
                balanceSheet.TaxPayable = valuesFinalList[index];
                balanceSheet.AccruedLiabilities = valuesFinalList[index];
                balanceSheet.ShortTermDebt = valuesFinalList[index];
                balanceSheet.DeferredRevenue = valuesFinalList[index];
                balanceSheet.OtherLiabilities = valuesFinalList[index];
                balanceSheet.TotalLiabilities = valuesFinalList[index];
                balanceSheet.RetainedEarnings = valuesFinalList[index];
                balanceSheet.CommonStock = valuesFinalList[index];
                balanceSheet.AOCI = valuesFinalList[index];
                balanceSheet.ShareholdersEquity = valuesFinalList[index];
                balanceSheet.LiabilitiesAndEquity = valuesFinalList[index];
            }

        }
        if (balanceSheet.Year != 0)
        {
            balanceSheetForCompany.Add(balanceSheet);
